...
def из_россии():
    try:
        ...
        if not rid:
            заметки.append("служба проверки не выдала задание — замер пропущен")
            return
        time.sleep(30)
        ...
        заметки.append(f"служба проверки недоступна ({type(e).__name__}) — замер пропущен")
        return
...
def не_замерено(всего):
    if всего == 0:
        заметки.append("служба проверки не вернула результатов — замер пропущен")
        return True
    return False
